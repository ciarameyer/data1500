...
# chart 1 -- attendance by team over time
...
# chart 2 -- attendance by team over time cumulative  
...
ggplot(no_takeover, aes(x = id, y = `Published Attendance`, 
                        fill = `Home Team`)) +
  geom_stream(type = "proportional", bw = 0.75) +
  geom_vline(xintercept = 162) +
  theme_minimal() +
  scale_y_continuous(labels = scales::comma) +
  labs(title = "Expansion teams are driving increased attendance at PWHL games",
...
# attempting monthly game average to see if that has smoothing impact
...
# trying another smoothing method -- lets look at share of attendance per month?
smoothed_2 <- no_takeover %>%
  ...
  group_by(id) %>%
  mutate(monthly_attendance = sum(`Published Attendance`)) %>%
  group_by(id, `Home Team`) %>%
  summarise(share_of_attendance_in_month = sum(`Published Attendance`) / first(monthly_attendance)) 

# sanity check
smoothed_2 %>%
  group_by(id) %>%  
  summarise(total_share = sum(share_of_attendance_in_month))

smoothed_2$`Home Team` <- factor(
  smoothed_2$`Home Team`,
  levels = c("Vancouver", "Seattle", "New York", "Boston", "Minnesota", "Toronto",
             "Ottawa", "Montréal")
)

ggplot(smoothed_2, aes(x = id, y = share_of_attendance_in_month, 
                     fill = `Home Team`)) +
...
# what it seems like from this initial analysis is that mens team popularity 
# generally coorelates with womens. SEA is a notable exception, with a consistently
# unpopular mens team, but the new womens team is very popular

# let's try and make this into a chart. 
mens_v1 <- mens %>%
  mutate(month = floor_date(Date, "month")) %>% 
  # cut off months that aren't in the women's data 
  ...
  group_by(id, `Home Team`) %>%
  summarise(games_in_month = n(),
            attendance_over_month = mean(Att., na.rm = TRUE)) %>%
  ungroup() %>%
  group_by(id) %>%
  mutate(rank = min_rank(attendance_over_month))

ggplot(mens_v1, aes(x = id, y = rank, color = `Home Team`)) +
  geom_bump(linewidth = 1.5) +
  geom_point(size = 3)

# chart 3 - bump of men v women's ranks 
...
ggplot(no_takeover, aes(x = id, y = `Percent of Venue Cap`, color = `Home Team`)) +
  geom_jitter() +
  geom_smooth(method = "lm", se = FALSE) +
  geom_vline(xintercept = 73) +
  geom_vline(xintercept = 163) +
  theme_minimal() +
  labs(title = "By the end of the 2025-26 season, more than half of PWHL teams were at about 90% capacity for home games",
       subtitle = "Percent of venue size filled at PWHL games, excluding the Takeover Tour",
       caption = "PWHL via Jenness Wayne") 


# chart 5 - just overall attendance
...
takeover_analysis <- full %>%
  group_by(Takeover) %>%
  summarise(average_attendance = mean(`Published Attendance`)) 

# toronto venue fill
toronto <- full %>%
  filter(`Home Team` == "Toronto")
